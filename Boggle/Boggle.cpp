...
Vector<string> GetOverridenRows()
{
	int inputSize = 16;
	int rowSize = 4;
...
Vector<string> GetDefaultBoardCubes()
{
...
void PlayBoogle(int minWordLength, string sentinel, Lexicon lexicon)
{
	Grid<char> board;

	if(IsAskingForOverride())
		board = GetOverridenBoard(GetOverridenRows());
	else 
		board = GetBoard(GetDefaultBoardCubes());
...
int main() 
{
	...
	do
		PlayBoogle(minWordLength, sentinel, Lexicon(lexiconPath));
