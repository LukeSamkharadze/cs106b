...
TextInfo GetTextInfoFromFile(string fileName, bool isDebugModeOn)
{
	...
	while (tokenScanner.hasMoreTokens()) 
	{
		...
		if(IsWord(token))
		{
			if(isDebugModeOn) cout << "Token: ["<< token <<"]  (word; " << CountSyllables(token) << ((CountSyllables(token) == 1 )? " syllable)":" syllables)" ) << endl;
		...
		{
			if(isDebugModeOn) cout << "Token: [" << token <<"]  (end of sentence)" << endl;
			textInfo.sentenceCount++;
		}
		else
			if(isDebugModeOn) cout << "Token: [" << token << "]"<<endl;
	}
...
int main() 
{
	...
	bool isDebugModeOn;
	cout << "Enter '1' or '0' if u want to enable debug mode or not: ";
	cin >> isDebugModeOn;

    TextInfo textInfo = GetTextInfoFromFile(fileName, isDebugModeOn);
